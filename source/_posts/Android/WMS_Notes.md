...
- DMS/CMS/TOF/CAN/App Adapter jar 包：启动 ZMQ，接收数据，存入 Ignite 缓存

- SensorHub：主要作用是将 KV 缓存中的实时 DMS 数据转换为结构化表记录

- DriverStatusManager：行为分析，分为实时分析（每 2 秒读取一次）和历史数据分析

    - 从 Ignite 的 `CANTABLE` 中查询特定时间窗口内的车辆数据。

    - 急加速急减速定义：不同速度区间对应不同的横纵向加速度阈值，超出阈值则定义急加速/急减速

    - 当检测到急加速/急减速开始时，在 `UBDTable` 中插入一条初始记录。

    - 当行为结束时，更新该记录的详细信息（持续时间、平均速度、平均加速度等）。

    - 检测到事件时，通过 `publishUBDEvent` 方法发布事件消息，内部通过 ZMQ 发布。

    - 核心算法

        ``` scss
        speed >= 0.5 && speed < 20  →  急加速阈值: 3.15, 急刹车阈值: -3.5
        speed >= 20 && speed < 60   →  急加速阈值: 3.15, 急刹车阈值: -2.7
        speed >= 60 && speed < 100  →  急加速阈值: 2.17, 急刹车阈值: -1.8
        speed >= 100 && speed < 120 →  急加速阈值: 1.22, 急刹车阈值: -1
        speed >= 120 && speed < 240 →  急加速阈值: 0.58, 急刹车阈值: -0.75
        speed >= 240 && speed < 1000→  急加速阈值: 0.44, 急刹车阈值: -0.56
        ```

        

- DemoCarEIIP(DataVisualizationFragment)：

    - 接收 DSM 通过 ZMQ 推送的实时驾驶行为事件
    - **地图显示**：高德地图显示行为发生的地理位置
        - **实时标记**：收到 ZMQ 事件时，在地图上添加标记点
        - **历史标记**：查询数据库后，一次性标记所有历史行为点
        - **交互联动**：点击散点图上的点，地图会自动定位到对应位置
    - **散点图**：展示行为事件在时间轴上的分布
    - **日历视图**：选择查看不同日期的历史数据

### AMS/WMS

AMS 和 WMS 运行在同一 system_server 进程内，通过 LocalServices 暴露的内部接口进行直接方法调用；AMS 在 Activity 生命周期和任务变化时通知 WMS 更新窗口状态，而 WMS 在窗口布局、显示、输入、配置变化等关键节点回调 AMS，以保持 Activity 状态与窗口状态的一致性。
